...
class App():
    ...
    def login(self):
        ...
        if self.login_username_field_active:
            username_field_color = 'lightblue'
        else:
            username_field_color = 'lightgray'

        if self.login_password_field_active:
            password_field_color = 'lightblue'
        else:
            password_field_color = 'lightgray'

        pygame.draw.rect(self.screen, username_field_color, self.login_username_input_field)
        ...
        pygame.draw.rect(self.screen, password_field_color, self.login_password_input_field)
    ...
    def verif_login(self):
        ...
        self.cursor.execute(f"SELECT id, username FROM users WHERE username = '{self.login_username_input}' AND password = '{sha256(self.pepper + self.login_password_input.encode() + salt).hexdigest()}';")
    ...
    def create_account(self):
        ...
        if self.username_exists():
            ...
            if self.password_is_valid():
                ...
                password = sha256(self.pepper + self.register_password_input.encode() + salt).hexdigest()
        ...
        for event in pygame.event.get():
            ...
            if app_state == "login":
                ...
                if event.type == pygame.KEYDOWN: # TODO need to simplify this
                    ...
                    if self.login_password_field_active:
                        if event.key == pygame.K_BACKSPACE:
                            self.login_password_input = self.login_username_input[:-1]
                        else:
